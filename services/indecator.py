...
def get_range_filter(source, sampling_period, multiplier):
    ...
    avrng = exp_moving_average(tr2['source'], sampling_period)
    smrng = exp_moving_average(avrng, (sampling_period*2-1)) * multiplier
    ...
    ft, upw, dnw = pd.Series(filt.iloc[:, 0]), pd.Series(upward), pd.Series(downward)
